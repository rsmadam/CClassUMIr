...
miR.arr.scaled$CMS <- expr.arr.CMS$predictedCMS$RF

miR.arr.scaled <- miR.arr.scaled[which(!is.na(miR.arr.scaled$CMS)),]
...
getBM( "ensembl_gene_id", 
       "affy_huex_1_0_st_v2", "33800", old_mart)

safM <- getBM(attributes=c('refseq_mrna',
                          'ensembl_gene_id',
                          'gene_biotype', 
                          'entrezgene_id'),
             filters = 'entrezgene_id',
             values = rownames(expr.arr.35834),
             mart = old_mart)

rownames(expr.arr.35834) <- make.names(safM$entrezgene_id[match(rownames(expr.arr.35834),
                                               safM$affy_huex_1_0_st_v2)], unique=T)
rownames(expr.arr.35834) <- sub("X", "",rownames(expr.arr.35834))
...
confusionMatrix(factor(pred_sc_gse35834$RF.nearestCMS), factor(pred_sc_gse35834$CMS.mRNA))
